...
mod test {
    ...
    use test;
    use traits::{Collection, Iter, MatrixMutDiag, MutIter, OptionIndex, OptionIndexMut};
    ...
    fn iter(size: (uint, uint), diag: int) -> TestResult {
        if let Some(d) = test::mat(size).as_mut().and_then(|m| m.mut_diag(diag)) {
            if diag > 0 {
    ...
    fn mut_iter(size: (uint, uint), diag: int) -> TestResult {
        if let Some(mut d) = test::mat(size).as_mut().and_then(|m| m.mut_diag(diag)) {
            if diag > 0 {
