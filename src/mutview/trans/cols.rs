...
mod test {
    ...
    use test;
    use traits::{Iter, MatrixCols, OptionMutSlice, Transpose};
    ...
    fn iter(
        ...
        if let Some(t) = test::mat(size).as_mut().and_then(|m| {
    ...
    macro_rules! sum {
        ($($ty:ident),+) => {$(
            mod $ty {
                ...
                use traits::{Iter, MatrixCols, MatrixRows, OptionMutSlice, SumCols, Transpose};
                ...
                fn sum(
                    ...
                    if let Some(t) = test::rand_mat::<$ty>(size).as_mut().and_then(|m| {
                        m.mut_slice(start, end)
                    }).map(|v| v.t()) {
                        let (nrows, _) = test::size(start, end);
