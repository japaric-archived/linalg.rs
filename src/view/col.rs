...
impl<'a, 'b, T> UnsafeMatrixCol<'b, &'b [T]> for View<'a, T> {
    unsafe fn unsafe_col(&'b self, col: uint) -> Col<&'b [T]> {
        let data = mem::transmute(raw::Slice {
            data: self.data.offset((col * self.stride) as int),
            len: self.nrows(),
        });
...
impl<'a, 'b, T> MatrixCol<'b, &'b [T]> for View<'a, T> {}
...
mod test {
    ...
    use test;
    use traits::{Iter, MatrixCol, OptionIndex, OptionSlice};
    ...
    fn iter(
        ...
        if let Some(c) = test::mat(size).as_ref().and_then(|m| {
            m.slice(start, end)
        }).as_ref().and_then(|v| {
            v.col(col)
        }) {
