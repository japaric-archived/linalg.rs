...
mod trans {
    macro_rules! blas {
        ($($ty:ident),+) => {$(mod $ty {
            ...
            // Test that `mul(Trans<Mat>)` is correct for `MutView`
            #[quickcheck]
            fn mat(
                start: (uint, uint),
            ...
            // Test that `mul(Trans<View>)` is correct for `MutView`
            #[quickcheck]
            fn view(
                ...
                test!({
                    ...
                    let lhs = try!(m.slice_from_mut(lhs_start));
                    let r = try!(lhs.row(row));
            ...
            // Test that `mul(Trans<MutView>)` is correct for `MutView`
            #[quickcheck]
            fn view_mut(
                (lhs_start, rhs_start): ((uint, uint), (uint, uint)),
                (m, k, n): (uint, uint, uint),
...
macro_rules! blas {
    ($($ty:ident),+) => {$(mod $ty {
        ...
        // Test that `mul(Mat)` is correct for `MutView`
        #[quickcheck]
        fn mat(
            start: (uint, uint),
            (m, k, n): (uint, uint, uint),
        ...
        // Test that `mul(View)` is correct for `MutView`
        #[quickcheck]
        fn view(
            (lhs_start, rhs_start): ((uint, uint), (uint, uint)),
            (m, k, n): (uint, uint, uint),
        ...
        // Test that `mul(MutView)` is correct for `MutView`
        #[quickcheck]
        fn view_mut(
            (lhs_start, rhs_start): ((uint, uint), (uint, uint)),
            (m, k, n): (uint, uint, uint),
