...
macro_rules! blas {
    ($($ty:ident),+) => {$(mod $ty {
        mod owned {
            ...
            // Test that `sub_assign(Row<Box<[T]>>)` is correct for `Row<Box<[T]>>`
            #[quickcheck]
            ...
            // Test that `sub_assign(T)` is correct for `Row<Box<[T]>>`
            #[quickcheck]
            ...
            // Test that `sub_assign(Row<&[T]>)` is correct for `Row<Box<[T]>>`
            #[quickcheck]
            ...
            // Test that `sub_assign(Row<&mut [T]>)` is correct for `Row<Box<[T]>>`
            #[quickcheck]
            ...
            // Test that `sub_assign(Row<strided::Slice>)` is correct for `Row<Box<[T]>>`
            #[quickcheck]
            ...
            // Test that `sub_assign(Row<strided::MutSlice>)` is correct for `Row<Box<[T]>>`
            #[quickcheck]
        ...
        mod slice_mut {
            ...
            // Test that `sub_assign(Row<Box<[T]>>)` is correct for `Row<&mut [T]>`
            #[quickcheck]
            ...
            // Test that `sub_assign(T)` is correct for `Row<&mut [T]>`
            #[quickcheck]
            ...
            // Test that `sub_assign(Row<&[T]>)` is correct for `Row<&mut [T]>`
            #[quickcheck]
            fn slice((nrows, ncols): (uint, uint), row: uint, idx: uint) -> TestResult {
                enforce! {
                    row < nrows,
            ...
            // Test that `sub_assign(Row<&mut [T]>)` is correct for `Row<&mut [T]>`
            #[quickcheck]
            fn slice_mut((nrows, ncols): (uint, uint), row: uint, idx: uint) -> TestResult {
                enforce! {
                    row < nrows,
            ...
            // Test that `sub_assign(Row<strided::Slice>)` is correct for `Row<&mut [T]>`
            #[quickcheck]
            fn strided((nrows, ncols): (uint, uint), row: uint, idx: uint) -> TestResult {
                enforce! {
                    row < nrows,
            ...
            // Test that `sub_assign(Row<strided::MutSlice>)` is correct for `Row<&mut [T]>`
            #[quickcheck]
            fn strided_mut((nrows, ncols): (uint, uint), row: uint, idx: uint) -> TestResult {
                enforce! {
                    row < nrows,
        ...
        mod strided_mut {
            ...
            // Test that `sub_assign(T)` is correct for `Row<strided::MutSlice>`
            #[quickcheck]
            fn owned((nrows, ncols): (uint, uint), row: uint, idx: uint) -> TestResult {
                enforce! {
                    row < nrows,
            ...
            // Test that `sub_assign(Row<&[T]>)` is correct for `Row<strided::MutSlice>`
            #[quickcheck]
            fn slice((nrows, ncols): (uint, uint), row: uint, idx: uint) -> TestResult {
                enforce! {
                    row < nrows,
            ...
            // Test that `sub_assign(Row<&mut [T]>)` is correct for `Row<strided::MutSlice>`
            #[quickcheck]
            fn slice_mut((nrows, ncols): (uint, uint), row: uint, idx: uint) -> TestResult {
                enforce! {
                    row < nrows,
            ...
            // Test that `sub_assign(Row<strided::Slice>)` is correct for `Row<strided::MutSlice>`
            #[quickcheck]
            fn strided((nrows, ncols): (uint, uint), row: uint, idx: uint) -> TestResult {
                enforce! {
                    row < nrows,
