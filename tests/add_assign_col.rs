...
macro_rules! blas {
    ($($ty:ident),+) => {$(mod $ty {
        mod owned {
            ...
            // Test that `add_assign(Col<Box<[T]>>)` is correct for `Col<Box<[T]>>`
            #[quickcheck]
            ...
            // Test that `add_assign(T)` is correct for `Col<Box<[T]>>`
            #[quickcheck]
            ...
            // Test that `add_assign(Col<&[T]>)` is correct for `Col<Box<[T]>>`
            #[quickcheck]
            ...
            // Test that `add_assign(Col<&mut [T]>)` is correct for `Col<Box<[T]>>`
            #[quickcheck]
            ...
            // Test that `add_assign(Col<strided::Slice>)` is correct for `Col<Box<[T]>>`
            #[quickcheck]
            ...
            // Test that `add_assign(Col<strided::MutSlice>)` is correct for `Col<Box<[T]>>`
            #[quickcheck]
        ...
        mod slice_mut {
            ...
            // Test that `add_assign(Col<Box<[T]>>)` is correct for `Col<&mut [T]>`
            #[quickcheck]
            ...
            // Test that `add_assign(T)` is correct for `Col<&mut [T]>`
            #[quickcheck]
            ...
            // Test that `add_assign(Col<&[T]>)` is correct for `Col<&mut [T]>`
            #[quickcheck]
            fn slice((nrows, ncols): (uint, uint), col: uint, idx: uint) -> TestResult {
                enforce! {
                    col < ncols,
            ...
            // Test that `add_assign(Col<&mut [T]>)` is correct for `Col<&mut [T]>`
            #[quickcheck]
            fn slice_mut((nrows, ncols): (uint, uint), col: uint, idx: uint) -> TestResult {
                enforce! {
                    col < ncols,
            ...
            // Test that `add_assign(Col<strided::Slice>)` is correct for `Col<&mut [T]>`
            #[quickcheck]
            fn strided((nrows, ncols): (uint, uint), col: uint, idx: uint) -> TestResult {
                enforce! {
                    col < ncols,
            ...
            // Test that `add_assign(Col<strided::MutSlice>)` is correct for `Col<&mut [T]>`
            #[quickcheck]
            fn strided_mut((nrows, ncols): (uint, uint), col: uint, idx: uint) -> TestResult {
                enforce! {
                    col < ncols,
        ...
        mod strided_mut {
            ...
            // Test that `add_assign(T)` is correct for `Col<strided::MutSlice>`
            #[quickcheck]
            fn owned((nrows, ncols): (uint, uint), col: uint, idx: uint) -> TestResult {
                enforce! {
                    col < ncols,
            ...
            // Test that `add_assign(Col<&[T]>)` is correct for `Col<strided::MutSlice>`
            #[quickcheck]
            fn slice((nrows, ncols): (uint, uint), col: uint, idx: uint) -> TestResult {
                enforce! {
                    col < ncols,
            ...
            // Test that `add_assign(Col<&mut [T]>)` is correct for `Col<strided::MutSlice>`
            #[quickcheck]
            fn slice_mut((nrows, ncols): (uint, uint), col: uint, idx: uint) -> TestResult {
                enforce! {
                    col < ncols,
            ...
            // Test that `add_assign(Col<strided::Slice>)` is correct for `Col<strided::MutSlice>`
            #[quickcheck]
            fn strided((nrows, ncols): (uint, uint), col: uint, idx: uint) -> TestResult {
                enforce! {
                    col < ncols,
