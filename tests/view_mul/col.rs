macro_rules! blas {
    ($($ty:ident),+) => {$(mod $ty {
        ...
        // Test that `mul(Col<Box<[T]>>)` is correct for `View`
        #[quickcheck]
        fn owned(
            start: (uint, uint),
            (m, k): (uint, uint),
        ...
        // Test that `mul(Col<&[T]>)` is correct for `View`
        #[quickcheck]
        fn slice(
            start: (uint, uint),
        ...
        // Test that `mul(Col<&mut [T]>)` is correct for `View`
        #[quickcheck]
        fn slice_mut(
            start: (uint, uint),
            (m, k, n): (uint, uint, uint),
        ...
        // Test that `mul(Col<strided::Slice>)` is correct for `View`
        #[quickcheck]
        fn strided(
            start: (uint, uint),
            (m, k, n): (uint, uint, uint),
        ...
        // Test that `mul(Col<strided::MutSlice>)` is correct for `View`
        #[quickcheck]
        fn strided_mut(
            start: (uint, uint),
            (m, k, n): (uint, uint, uint),
