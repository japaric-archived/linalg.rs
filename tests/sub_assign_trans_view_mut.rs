...
mod trans {
    macro_rules! blas {
        ($($ty:ident),+) => {$(mod $ty {
            ...
            // Test that `sub_assign(Trans<View>)` is correct for `Trans<MutView>`
            #[quickcheck]
            fn view(
...
macro_rules! blas {
    ($($ty:ident),+) => {$(mod $ty {
        ...
        // Test that `sub_assign(T)` is correct for `Trans<MutView>`
        #[quickcheck]
        fn scalar(
            start: (usize, usize),
            (nrows, ncols): (usize, usize),
        ...
        // Test that `sub_assign(View)` is correct for `Trans<MutView>`
        #[quickcheck]
        fn view(
            (lhs_start, rhs_start): ((usize, usize), (usize, usize)),
            (nrows, ncols): (usize, usize),
