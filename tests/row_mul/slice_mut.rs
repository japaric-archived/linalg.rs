mod trans {
    macro_rules! blas {
        ($($ty:ident),+) => {$(mod $ty {
            ...
            // Test that `mul(Trans<Mat>)` is correct for `Row<&mut [T]>`
            #[quickcheck]
            ...
            // Test that `mul(Trans<View>)` is correct for `Row<&mut [T]>`
            #[quickcheck]
            fn view(
                start: (uint, uint),
            ...
            // Test that `mul(Trans<MutView>)` is correct for `Row<&mut [T]>`
            #[quickcheck]
            fn view_mut(
                start: (uint, uint),
                (m, k, n): (uint, uint, uint),
...
macro_rules! blas {
    ($($ty:ident),+) => {$(mod $ty {
        ...
        // Test that `mul(Mat)` is correct for `Row<&mut [T]>`
        #[quickcheck]
        fn mat((m, k, n): (uint, uint, uint), row: uint, idx: uint) -> TestResult {
            enforce! {
                k != 0,
        ...
        // Test that `mul(View)` is correct for `Row<&mut [T]>`
        #[quickcheck]
        fn view(
            start: (uint, uint),
            (m, k, n): (uint, uint, uint),
        ...
        // Test that `mul(MutView)` is correct for `Row<&mut [T]>`
        #[quickcheck]
        fn view_mut(
            start: (uint, uint),
            (m, k, n): (uint, uint, uint),
