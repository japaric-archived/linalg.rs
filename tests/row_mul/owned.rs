mod trans {
    macro_rules! blas {
        ($($ty:ident),+) => {$(mod $ty {
            ...
            // Test that `mul(Trans<View>)` is correct for `Row<Box<[T]>>`
            #[quickcheck]
            fn view(
                start: (uint, uint),
                (k, n): (uint, uint),
            ...
            // Test that `mul(Trans<MutView>)` is correct for `Row<Box<[T]>>`
            #[quickcheck]
            fn view_mut(
                start: (uint, uint),
                (k, n): (uint, uint),
...
macro_rules! blas {
    ($($ty:ident),+) => {$(mod $ty {
        ...
        // Test that `mul(View)` is correct for `Row<Box<[T]>>`
        #[quickcheck]
        fn view(
            start: (uint, uint),
            (k, n): (uint, uint),
        ...
        // Test that `mul(MutView)` is correct for `Row<Box<[T]>>`
        #[quickcheck]
        fn view_mut(
            start: (uint, uint),
            (k, n): (uint, uint),
