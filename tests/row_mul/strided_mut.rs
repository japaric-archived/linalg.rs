mod trans {
    macro_rules! blas {
        ($($ty:ident),+) => {$(mod $ty {
            ...
            // Test that `mul(&Trans<Mat>)` is correct for `strided::MutRow`
            #[quickcheck]
            ...
            // Test that `mul(Trans<View>)` is correct for `strided::MutRow`
            #[quickcheck]
            fn view(
                start: (usize, usize),
            ...
            // Test that `mul(&Trans<MutView>)` is correct for `strided::MutRow`
            #[quickcheck]
            fn view_mut(
                start: (usize, usize),
                (m, k, n): (usize, usize, usize),
...
macro_rules! blas {
    ($($ty:ident),+) => {$(mod $ty {
        ...
        // Test that `mul(&Mat)` is correct for `strided::MutRow`
        #[quickcheck]
        fn mat((m, k, n): (usize, usize, usize), row: usize, idx: usize) -> TestResult {
            enforce! {
                k != 0,
        ...
        // Test that `mul(View)` is correct for `strided::MutRow`
        #[quickcheck]
        fn view(
            start: (usize, usize),
            (m, k, n): (usize, usize, usize),
        ...
        // Test that `mul(&MutView)` is correct for `strided::MutRow`
        #[quickcheck]
        fn view_mut(
            start: (usize, usize),
            (m, k, n): (usize, usize, usize),
