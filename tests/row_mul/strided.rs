mod trans {
    macro_rules! blas {
        ($($ty:ident),+) => {$(mod $ty {
            ...
            // Test that `mul(Trans<View>)` is correct for `Row<strided::Slice>`
            #[quickcheck]
            fn view(
                start: (uint, uint),
            ...
            // Test that `mul(Trans<MutView>)` is correct for `Row<strided::Slice>`
            #[quickcheck]
            fn view_mut(
                start: (uint, uint),
                (m, k, n): (uint, uint, uint),
...
macro_rules! blas {
    ($($ty:ident),+) => {$(mod $ty {
        ...
        // Test that `mul(View)` is correct for `Row<strided::Slice>`
        #[quickcheck]
        fn view(
            start: (uint, uint),
            (m, k, n): (uint, uint, uint),
        ...
        // Test that `mul(MutView)` is correct for `Row<strided::Slice>`
        #[quickcheck]
        fn view_mut(
            start: (uint, uint),
            (m, k, n): (uint, uint, uint),
